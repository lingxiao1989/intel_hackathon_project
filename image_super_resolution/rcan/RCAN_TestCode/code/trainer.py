...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        self.sycl = args.sycl
        if self.sycl:
            self.model = self.model.to('dpcpp')
    ...
    def test(self):
        ...
        self.ckp.write_log(
            'Total time: {:.2f}s, ave time: {:.2f}s\n'.format(time_d, time_d / (len(self.loader_test) - self.args.warmup), refresh=True)
        )
    ...
    def prepare(self, l, volatile=False):
        ...
        def _prepare(tensor):
            ...
            if self.args.precision == 'bfloat16': tensor = tensor.bfloat16()
